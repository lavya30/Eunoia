...
- Billing & Stripe: `GET /api/billing/prices`, `POST /api/billing/checkout` (requires auth; returns checkout session URL), `POST /api/billing/portal` (requires active subscription; returns customer portal URL), `GET /api/billing/subscription` (requires auth; returns subscription record & tier), `POST /api/billing/webhook` (raw-body webhook intercepted in `routes.ts` bridge before Elysia parsing; verifies HMAC/Stripe signature and mutates `User.tier`)
